...
* The workflow MUST mint a short-lived installation token using SHA-pinned `actions/create-github-app-token@v3.2.0`, `secrets.RELEASE_PLEASE_APP_CLIENT_ID`, and `secrets.RELEASE_PLEASE_APP_PRIVATE_KEY`.
...
* There is still a concurrency lane (`group: release-please`, `cancel-in-progress: false`) so two rapid `push` events do not mutate the same Release PR/manifest at the same time. This is not a substitute for the App-backed CI gating on the Release PR itself, but it prevents release-please runs from overlapping each other when the branch is updated quickly.
...
* Repository secrets named `RELEASE_PLEASE_APP_CLIENT_ID` and `RELEASE_PLEASE_APP_PRIVATE_KEY` are required. The Client ID is not sensitive but is accepted as a secret to match the repository's configured credential storage; the private key remains the long-lived credential. The App must be installed only on `resume-builder`, grant only Contents/Pull requests/Issues write, and have no ruleset bypass. The workflow exposes only a repository-scoped installation token that expires after about one hour.
...
* NFR-001: The release workflow disables repository permissions on its ambient `GITHUB_TOKEN` and grants release capabilities only through a short-lived GitHub App installation token scoped to this repository and the minimum required permissions (`contents: write`, `pull-requests: write`, `issues: write`).
  * Objective threshold or evaluation condition: the workflow's top-level `permissions:` value is exactly `{}`; the App-token step explicitly limits `owner` and `repositories` to this repository, requests exactly the three required write permissions, and the release-please step consumes only that ephemeral step output.
...
* `actions/create-github-app-token@v3.2.0` (pinned by SHA `bcd2ba49218906704ab6c1aa796996da409d3eb1`): mints a one-hour installation token from repository secrets `RELEASE_PLEASE_APP_CLIENT_ID` and `RELEASE_PLEASE_APP_PRIVATE_KEY`, scoped to this repository and the three required write permissions.
