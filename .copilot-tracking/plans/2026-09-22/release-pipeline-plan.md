...
* The stale-run guard MUST fail the run (non-zero exit) rather than silently treat a failed `git ls-remote` lookup as "stale, skip" — an empty/failed lookup means the guard cannot determine whether the run is current, and treating that ambiguity as a silent skip would drop a legitimate release update with no failure signal.
...
* Fail-loud fix, added after a third Copilot code review: the guard's `git ls-remote` result was not checked for emptiness, so a transient network/API failure produced an empty `current_sha`, which never equals `WORKFLOW_RUN_HEAD_SHA` and was therefore misclassified as "stale, skip" — silently dropping a legitimate release update with no error signal. The step now sets `shell: bash` with `set -euo pipefail` and explicitly checks for an empty `current_sha`, exiting non-zero (failing the job) instead of guessing.
