...
| `python3 -c "yaml.safe_load(...)"` | `.github/workflows/release-please.yml` | Passed | Valid YAML; the final post-CCR design triggers on pushes to `main`, disables default `GITHUB_TOKEN` permissions, mints a repository-scoped installation token through SHA-pinned `actions/create-github-app-token@v3.2.0`, requests only Contents/Pull requests/Issues write, and passes the ephemeral token to SHA-pinned `release-please-action@v5.0.0`. See the changes record for the superseding validation. |
...
| FR-003 (workflow runs on push to `main`) | `.github/workflows/release-please.yml` uses `push: branches: [main]`; the generated Release PR uses a short-lived GitHub App installation token so its updates can trigger normal PR CI instead of being suppressed by `GITHUB_TOKEN` anti-recursion | Met | Direct file inspection and hardened workflow validation |
...
| FR-005 (ruleset enforces linear history) | The consolidated "Protect main" ruleset (ID `23698833`) is active on the default branch and includes `required_linear_history`; the superseded standalone ruleset `23850509` was deleted | Met | Live API verification of the final consolidated ruleset state |
| NFR-001 (minimum workflow permissions) | Workflow declares `permissions: {}` for the ambient `GITHUB_TOKEN`; SHA-pinned `actions/create-github-app-token` mints a short-lived token scoped to this repository and exactly Contents/Pull requests/Issues write | Met | Direct file inspection and `scripts/validate_repo.py` hardened-token contract validation |
...
| P02-T01 (workflow) | All final binding-contract clauses verified: empty default permissions, SHA-pinned token/release actions, explicit repository and permission scope, ephemeral App-token handoff, concurrency, and explicit config/manifest inputs | Met | Direct file inspection, `scripts/validate_repo.py`, and focused structural tests |
| P03-T01 (ruleset) | "Protect main" (ID `23698833`) is active, includes `required_linear_history` and the required `validate` check, and has no bypass actors; squash/rebase merge remain enabled repository-wide | Met | Live `gh api` verification of the consolidated ruleset and repository merge-strategy settings |
