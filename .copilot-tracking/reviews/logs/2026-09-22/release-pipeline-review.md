...
| `python3 -c "yaml.safe_load(...)"` | `.github/workflows/release-please.yml` | Passed | Valid YAML; permissions (`contents`, `pull-requests`, `issues` only), trigger (`push: branches: [main]`), action pin, and `with:` inputs all match P02-T01's binding contract |
...
| FR-003 (workflow runs on push to `main`) | `.github/workflows/release-please.yml` trigger is `push: branches: [main]` only | Met | Direct file inspection |
