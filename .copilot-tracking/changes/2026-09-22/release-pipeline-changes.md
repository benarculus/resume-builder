...
* Validation: passed — reviewed Linux/Python 3.12 wheel lock downloaded with `--require-hashes --only-binary=:all:`; focused suite 73 passed with official integration active; hosted Python 3.12 CI 100 passed with no skips; local Python 3.12 full suite 92 passed, 8 system-binary skips; local Python 3.9 full suite 87 passed, 13 expected skips; `python3 scripts/validate_repo.py`; checksum-verified Syft `v1.52.0` generate → prepare → official validate → contract validate; and `git diff --check`.
