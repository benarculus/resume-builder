...
* Behavior or functionality changed: `release-please-action@v5.0.0` (pinned to commit SHA `45996ed1f6d02564a971a2fa1b5860e934307cf7`, matching the repository's existing action-pinning convention from `ci.yml`) now runs via `workflow_run`, chained after `ci.yml` ("Validate resume-builder") completes on a push to `main` with `conclusion == 'success'`, and opens or updates a standing Release PR from Conventional Commit history. **Correction (post-CCR review):** the workflow was originally implemented with a bare `push: branches: [main]` trigger and a claim that the default `GITHUB_TOKEN` was fully sufficient. Copilot code review correctly flagged that GitHub's anti-recursion rule prevents `GITHUB_TOKEN`-authored events (release-please's own generated branch-push and Release-PR-open) from retriggering `pull_request`-scoped workflows such as `ci.yml`/`codeql-analysis.yml` — meaning those checks never ran on the Release PR itself under the original trigger. After discussing token-based (PAT/App) versus trigger-based mitigations with the user, the workflow was changed to trigger via `workflow_run` instead, so release-please only ever proposes or advances the Release PR once CI has already passed on `main` — no PAT/App token was added. **Correction 2 (post-CCR review, second pass):** Copilot code review then correctly flagged a race condition in the `workflow_run` design: the event carries the SHA CI validated, not necessarily the current tip of `main` at the moment the workflow runs, so a delayed CI completion for an earlier commit could fire after a later commit has already landed, letting release-please act on that superseded (and not-yet-validated) tip. Added a `Guard against a stale CI run` step that compares `github.event.workflow_run.head_sha` against the live `refs/heads/main` tip via `git ls-remote` and only invokes `release-please-action` when they match, otherwise the run is a no-op. **Correction 3 (post-CCR review, third pass):** Copilot code review then correctly flagged that the stale-run guard only filters which SHA a given run acts on — it does not stop two separately-triggered `workflow_run` completions (e.g. a manual CI rerun on the same commit, or a later push whose CI finishes while an earlier run is still mid-execution) from invoking `release-please-action` at the same time, which could race on mutating the same Release PR/manifest. Added a job-level `concurrency: { group: release-please, cancel-in-progress: false }` so every invocation is queued onto a single lane and only one ever mutates release state at a time. The workflow still declares exactly `permissions: { contents: write, pull-requests: write, issues: write }` (no broader scope), still relies on the default `secrets.GITHUB_TOKEN` with no custom token input, and still explicitly references `config-file: release-please-config.json` and `manifest-file: .release-please-manifest.json`. **Correction 4 (post-CCR review, fourth pass):** Copilot code review then correctly flagged that a failed `git ls-remote` lookup in the stale-run guard produced an empty `current_sha`, which never matched `WORKFLOW_RUN_HEAD_SHA` and was silently treated as "stale, skip" -- dropping a legitimate release update with no failure signal on a transient network/API error. Added `shell: bash` with `set -euo pipefail` and an explicit empty-`current_sha` check that exits non-zero instead of guessing, so a lookup failure now fails the job loudly rather than silently skipping. Merging the resulting Release PR will update `CHANGELOG.md`, tag the merge commit, and create a GitHub Release — none of which existed before this change.
* Validation: passed for structure — YAML parses successfully (`python3 -c "import yaml; yaml.safe_load(...)"`); permissions, trigger (`workflow_run` on `ci.yml` completion, gated on `conclusion == 'success'`, `head_branch == 'main'`, `event == 'push'`), the stale-run guard step, action pin, and `with:` inputs all match the plan's corrected binding contract. Skipped for `actionlint`/`yamllint` — neither tool is installed in this environment, and no repository-level linter config exists to fall back to. Unavailable/unverified: an actual live workflow run against a real push to `main` (and a real `ci.yml` completion event), including the stale-run guard's behavior under a genuine race between two rapid commits, which is the practical acceptance check the plan calls out as untested until the pipeline's first real run (per the plan's Confidence and uncertainty note).
