...
Adds an automated `release-please` pipeline so `resume-builder` tracks version updates semantically instead of by hand. Pushes to `main` will open/update a release PR that bumps the version based on Conventional Commit messages and keeps `plugin.json` and `.github/plugin/marketplace.json` in sync.
...
- `.github/workflows/release-please.yml` — triggers on push to `main`, pinned to `googleapis/release-please-action@v5.0.0` (commit-SHA pinned per repository convention)
