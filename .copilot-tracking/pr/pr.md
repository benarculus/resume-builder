...
- `.github/workflows/publish-release.yml` — reacts to the App-created version tag, waits for release-please's draft release, generates and validates an SPDX 2.3 SBOM with read-only permissions, transfers only the validated SBOM into a separate publication job, checksum-verifies the uploaded asset, and publishes the immutable release
...
- [x] `python3 -m pytest -q` — 67 passed, 8 skipped using the local Python 3.9 environment; hosted CI remains authoritative for the repository-pinned Python 3.12 dependency set
